...
responce <- GET(paste0(base.uri, resource))

# Extract the "text" of the response usin the `content` function
responce.content <- content(responce, "text")

# Convert the body from JSON into a data frame
body.data <- fromJSON(responce.content)
